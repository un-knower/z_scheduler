 ...
 */
	...
	function executeTask(id,sync){
		if(sync!=false){
			sync=true;
		}
		var urlPath="./task?op=exec&id="+id+"&taskdate=${taskdate}"
		$.ajax({
			url : urlPath,
			data: "",
			dataType : "html",
	...
	function fillTable( tasks ) {
		...
		for(var i=0;i<tasks.length;i++ ){
			...
			itemline+="<td id='batchid_'"+t.id+">"+t.batchid+"</td>";
			itemline+="<td id='tasktype_'"+t.id+">"+t.tasktype+"</td>";
			...
			itemline+="<td> <a href='javascript:executeTask("+t.id+")'> 执行</a> <a href='"+log_path+"'> 日志</a></td>";
