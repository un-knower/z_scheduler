...
public class TaskService {
	...
	//根据id执行,界面方式
	public Map<String,String> executeTask(Task task,String taskdate) {
		Map<String,String> resultMap = null;
		String cmd = getTrueCommand(task);
		task.setFlag(Constants.BATCH_RUNNING);
		updateTask(task);
		if(task.getTaskhost().equals("10.201.48.101")){
			resultMap = SshUtil.execTest(cmd);
		}else{
			resultMap = SshUtil.execProduct(cmd);
		}
	...
	public String getTrueCommand(Task task){
		...
		if(task.getTaskhost().equals("10.201.48.101")){
			...
			//非本地执行
			true_command = "sudo -S -p 'bl' -u hive "+task.getCommandpath()+task.getCommand()+" "+args;
		}
