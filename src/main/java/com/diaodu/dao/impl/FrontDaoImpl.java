...
public class FrontDaoImpl implements FrontDao {
	...
	public List<Front> getAllFront() throws SQLException {
		...
		List<Front> list = queryRunner.query(connection,"select * from front ", new BeanListHandler<>(Front.class));
		List<ETL> list2 = queryRunner.query(connection,"select * from etl  order by etl_type", new BeanListHandler<>(ETL.class));
		return list;
	}
