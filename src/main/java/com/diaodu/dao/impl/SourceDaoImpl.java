...
public class SourceDaoImpl implements SourceDao {
	...
	public int addSource(Source s) throws SQLException {
		Connection connection = JDBCUtils.getConnection();
		QueryRunner queryRunner = new QueryRunner();
		String sql="insert into source(db_ip,db_name,db_port,hive_table_name,is_inc"
				+ ",map_sign,password,remark,system_id,table_name,user_name)"+
		" values (?,?,?,?,?,?,?,?,?,?)";
		Object[] params={s.getDb_ip(),s.getDb_name(),s.getDb_port(),s.getHive_table_name(),s.getIs_inc()
				,s.getMap_sign(),s.getPassword(),s.getRemark(),s.getSystem_id(),s.getTable_name()
				,s.getUser_name()};
